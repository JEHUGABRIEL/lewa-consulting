...
import { featuredFormations, type FeaturedFormation, type FormationRow } from "@/lib/formations";
import type { ServiceItem } from "@/lib/services";
import type { Post } from "@/lib/posts";
import type { Partner } from "@/lib/partners";
import type { PublicTestimonial } from "@/lib/admin/public";
...
const FEATURED_CARD_KEYS = [
  { nameKey: "formations.featured1Name", descKey: "formations.featured1Desc" },
  { nameKey: "formations.featured2Name", descKey: "formations.featured2Desc" },
  { nameKey: "formations.featured3Name", descKey: "formations.featured3Desc" },
  { nameKey: "formations.featured4Name", descKey: "formations.featured4Desc" },
] as const;

const featuredIndexBySlug = new Map(
  featuredFormations.map((f, i) => [f.slug, FEATURED_CARD_KEYS[i]] as const),
);
...
function NewBadge() {
  ...
  return (
    <span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full bg-gold px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-navy shadow-sm">
      <svg
        className="h-3 w-3"
        viewBox="0 0 24 24"
        fill="none"
...
function useVisibleCount(mobile: number, tablet: number, desktop: number) {
  ...
  useEffect(() => {
    ...
    const update = () =>
      setCount(mobileMQ.matches ? mobile : tabletMQ.matches ? tablet : desktop);
    update();
    ...
    <figure className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-transparent bg-white px-7 py-9 text-center shadow-[0_1px_3px_rgba(12,35,64,0.05)] transition-all duration-500 hover:-translate-y-1 hover:border-gold/40 hover:shadow-[0_18px_44px_rgba(12,35,64,0.10)] sm:px-8">
      ...
      <figcaption className="relative mt-7 flex flex-col items-center gap-3 border-t border-border/70 pt-6">
        ...
        <span>
          ...
          {testimonial.org && (
            <p className="mt-1 text-[11px] font-semibold uppercase tracking-wider text-gold">{testimonial.org}</p>
          )}
        </span>
      </figcaption>
    </figure>
...
function TestimonialCarousel({ testimonials, tr }: { testimonials: PublicTestimonial[]; tr: TrFn }) {
          ...
          {pages.map((page, i) => (
            <div key={i} className="w-full shrink-0 px-2 sm:px-3" aria-hidden={i !== current}>
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 lg:gap-8">
                {page.map((tm) => (
                  <TestimonialCard key={tm.name} testimonial={tm} />
                ))}
              </div>
            </div>
          ))}
...
export default function HomePage({
  ...
  const visibleCount = useVisibleCount(3, 4, 3);
    ...
    <main>
      ...
      <section>
        <Container className="py-14 sm:py-16">
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 stagger-children">
            {featured.slice(0, visibleCount).map((f) => {
              const keys = featuredIndexBySlug.get(f.slug);
              const nameKey =
                keys?.nameKey ?? `formations.items.${f.slug}.name`;
              const cardName = t(nameKey);
              const descKey =
                keys?.descKey ?? `formations.items.${f.slug}.desc`;
              const cardDesc = t(descKey);
              const showDesc =
                cardDesc && cardDesc !== descKey && cardDesc !== cardName;
              return (
                <Reveal key={f.slug} as="div">
                  <div className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border/70 bg-white shadow-[0_1px_3px_rgba(12,35,64,0.06)] transition-all duration-500 hover:-translate-y-1 hover:border-gold/40 hover:shadow-[0_18px_44px_rgba(12,35,64,0.10)]">
                    <Link
                    ...
                    <div className="flex flex-1 flex-col p-5">
                      ...
                      <h3 className="mt-3 font-display text-lg font-semibold leading-snug text-navy transition-colors duration-200 group-hover:text-red">
                        {cardName}
                      </h3>

                      {showDesc && (
                        <p className="mt-2 text-xs leading-relaxed text-ink/60 line-clamp-3">
                          {cardDesc}
                        </p>
                      )}
                      ...
                      <div className="mt-auto flex items-end justify-between gap-3 border-t border-border/70 pt-4">
                        ...
                        <button
                          type="button"
                          onClick={() => openEnrollment(f.slug)}
                          className="relative z-20 inline-flex shrink-0 items-center gap-1.5 rounded-full bg-gold px-4 py-2 text-xs font-semibold text-navy shadow-sm transition-all duration-300 group-hover:bg-navy group-hover:text-paper group-hover:shadow-md"
                        >
              ...
              <Reveal key={post.slug} as="div" delay={i * 60}>
                <div className="group relative flex h-full flex-col overflow-hidden rounded-xl bg-white shadow-sm hover-lift">
                  ...
                  <div className="flex flex-1 flex-col p-5 lg:p-6">
                    ...
                    { }
                    <h3 className="mt-1.5 font-display text-base font-semibold text-navy leading-snug transition-colors duration-200 group-hover:text-red">
      ...
      <section>
        <Container className="py-14 sm:py-16">
          ...
          <TestimonialCarousel testimonials={testimonials} tr={t} />
        </Container>
      </section>
      ...
      <EnrollmentModal
        ...
        formations={formations.map((f) => ({
          slug: f.slug,
          name: t(`formations.items.${f.slug}.name`),
        }))}
